...
import cn.cl.cyclamen.service.admin.RoleService;
...
public class StaffController {
    ...
    public Map<String, String> add(Staff staff){
        ...
        if(isExist(staff.getUsername(), 0l)){
            ret.put("type", "error");
            ret.put("msg", "该员工名已经存在，请重新输入！");
            return ret;
        }
        if(staffService.add(staff) <= 0){
            ret.put("type", "error");
            ret.put("msg", "员工添加失败，请联系管理员！");
            return ret;
        }
...
//		}
        ...
        if(isExist(staff.getUsername(), staff.getId())){
            ret.put("type", "error");
            ret.put("msg", "该员工名已经存在，请重新输入！");
            return ret;
        }
    ...
    public Map<String, String> delete(String ids){
        ...
        if(staffService.delete(ids) <= 0){
            ret.put("type", "error");
            ret.put("msg", "员工删除失败，请联系管理员！");
            return ret;
        }
        ret.put("type", "success");
        ret.put("msg", "员工删除成功！");
    ...
    private boolean isExist(String username,Long id){
        ...
        if(staff.getId().longValue() == id.longValue())return false;
        return true;
    }
}
