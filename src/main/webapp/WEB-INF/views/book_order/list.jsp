	...
	<form id="add-form" method="post">
        <table>
            <tr>
                ...
                <td>
	                <select id="add-userId" name="userId" class="easyui-combobox" panelHeight="auto" style="width:268px" data-options="required:true, missingMessage:'请选择客户'">
		            	<c:forEach items="${userList}" var="user">
		            	<option value="${user.id }" real-name="${user.realName }" id-card="${user.idCard }" mobile="${user.mobile }">${user.name }</option>
		            	</c:forEach>
	            	</select>	
                </td>
            </tr>
            ...
            <tr>
                ...
                <td><input type="text" id="add-name" name="realName" class="wu-text easyui-validatebox" data-options="required:true, missingMessage:'请输入真实姓名'" /></td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td>
                    <select id="add-payWay" name="payWay" class="easyui-combobox" panelHeight="auto" style="width:268px" data-options="required:true, missingMessage:'请选择'">
            ...
            <tr>
                ...
                <td>
                    <select id="add-orderWay" name="orderWay" class="easyui-combobox" panelHeight="auto" style="width:268px" data-options="required:true, missingMessage:'请选择'">
                        <option value="0">网站预订</option>
                        <option value="1">在店预订</option>
	...
	<form id="edit-form" method="post">
        ...
        <table>
           <tr>
                <td align="right">客户:</td>
                <td>
	                <select id="edit-userId" name="userId" class="easyui-combobox" panelHeight="auto" style="width:268px" data-options="required:true, missingMessage:'请选择客户'">
		            	<c:forEach items="${userList}" var="user">
		            	<option value="${user.id }" real-name="${user.realName }" id-card="${user.idCard }" mobile="${user.mobile }">${user.name }</option>
		            	</c:forEach>
	            	</select>	
            ...
            <tr>
                <td align="right">入住人姓名:</td>
                <td><input type="text" id="edit-name" name="realName" class="wu-text easyui-validatebox" data-options="required:true, missingMessage:'请输入姓名'" /></td>
            </tr>
            <tr>
                <td align="right">身份证号:</td>
...
<script type="text/javascript">
	...
	function openEdit(){
		...
		$('#edit-dialog').dialog({
            ...
            onBeforeOpen:function(){
                ...
                $("#edit-payWay").val(item.payWay);
                $("#edit-orderWay").val(item.orderWay);
            }
        });
	}
	...
	*/
	...
	$("#search-btn").click(function(){
		...
		var user = $("#search-user").combobox('getValue');
	...
	$("#add-userId").combobox({
		onSelect:function(data){
			//console.log(data);
			var option = $("#add-userId option[value='"+data.value+"']");
	...
	*/
	$('#data-datagrid').datagrid({
		...
		columns:[[
			...
			{ field:'userId',title:'客户',width:100,formatter:function(value,row,index){
			...
			{ field:'roomTypeId',title:'房型',width:150,formatter:function(value,row,index){
				var roomTypeList1 = $("#search-roomType").combobox('getData');
				for(var i=0;i<roomTypeList1.length;i++){
					if(roomTypeList1[i].value == value){
						return roomTypeList1[i].text /*'(￥:' + $("#search-roomType option[value='"+value+"']").attr('price') + ')'*/;
					}
					row.B=value;
				}
				return value;
			}},
            { field:'B',title:'价格',width:100,formatter:function(value,row,index){
                    var roomTypeList1 = $("#search-roomType").combobox('getData');
                    for(var i=0;i<roomTypeList1.length;i++){
                        if(roomTypeList1[i].value == value){
                            return  $("#search-roomType option[value='"+value+"']").attr('price') ;
                        }
                    }
                    return value;
                }},
            ...
            { field:'payWay',title:'支付方式',width:100,formatter:function(value,row,index){
                    switch(value){
                        case 0:{
                            return '支付宝';
                        }
            ...
            { field:'orderWay',title:'预订方式',width:100,formatter:function(value,row,index){
                    switch(value){
                        case 0:{
                            return '网站预约';
                        }
