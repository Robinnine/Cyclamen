	...
	<form id="add-form" method="post">
        <table>
	                ...
	                <select id="add-roomId" name="roomId" class="easyui-combobox" panelHeight="auto" style="width:268px" data-options="required:true, missingMessage:'请选择房间'">
		            	<c:forEach items="${roomList}" var="room">
                            <c:if test="${room.status == 0 }">
                                <option value="${room.id }">${room.sn }(${room.remark })</option>
                        </c:if>
		            	</c:forEach>
            ...
            <tr>
                ...
                <td><input type="text" id="add-realName" name="realName" class="wu-text easyui-validatebox" data-options="required:true, missingMessage:'请输入姓名'" /></td>
            	<td></td>
            </tr>
            <tr>
	...
	<form id="edit-form" method="post">
        ...
        <table>
           ...
           <tr>
                <td align="right">房间:</td>
                <td>
	                <select id="edit-roomId" name="roomId" class="easyui-combobox" panelHeight="auto" style="width:268px" data-options="required:true, missingMessage:'请选择房间'">
		            	<c:forEach items="${roomList}" var="room">
		            	<option value="${room.roomTypeId }">${room.sn }</option>
		            	</c:forEach>
	            	</select>
                </td>
            </tr>
            ...
            <tr>
                <td align="right">入住人姓名:</td>
                <td><input type="text" id="edit-realName" name="realName" class="wu-text easyui-validatebox" data-options="required:true, missingMessage:'请输入姓名'" /></td>
            </tr>
            <tr>
                <td align="right">身份证号:</td>
...
<script type="text/javascript">
	...
	*/
	...
	$("#search-btn").click(function(){
		var option = {realName:$("#search-name").val()};
		var status = $("#search-status").combobox('getValue');
		var user = $("#search-room").combobox('getValue');
	...
	*/
	$('#data-datagrid').datagrid({
		...
		columns:[[
            ...
            /*{ field:'B',title:'房间价格',width:100,formatter:function(value,row,index){
                    var roomTypeList1 = $("#search-roomType").combobox('getData');
                    for(var i=0;i<roomTypeList1.length;i++){
                        if(roomTypeList1[i].value == value){
			...
			{ field:'realName',title:'入住人姓名',width:100,sortable:true},
		...
		$('#show-order-dialog').dialog({
            ...
            buttons: [{
                ...
                handler: function(){
            		...
            		$("#add-realName").val(selectedOrder.realName);
                	...
                	$("#edit-realName").val(selectedOrder.realName);
